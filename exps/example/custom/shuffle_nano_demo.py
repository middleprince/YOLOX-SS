...
class Exp(MyExp):
    ...
    def get_model(self, sublinear=False):
        ...
        if "model" not in self.__dict__:
            from yolox.models import YOLOXSHead, YOLOSPAFPN, YOLOXS
            in_channels = [128, 256, 512]
            backbone = YOLOSPAFPN(self.depth, self.width, in_channels=in_channels, depthwise=True)
            head = YOLOXSHead(self.num_classes, self.width, in_channels=in_channels, depthwise=True)
            self.model = YOLOXS(backbone, head)
